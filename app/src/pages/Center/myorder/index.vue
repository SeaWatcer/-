    ...
    <div class="order-right">
        <div class="order-content">
            ...
            <div class="orders">
                ...
                <table class="order-item" v-for="order in orderData.records">
                    <thead>
                        <tr>
                            <th colspan="5">
            ...
            <div class="choose-order">
                ...
                <!-- 分页器 -->
                <Pagination :pageNo="page" :pageSize="limit" :total="orderData.total" :continues="5"
                    @getpageNo="getpageNo" />
            </div>
        </div>
        <!--猜你喜欢-->
...
export default {
    ...
    methods: {
        // 获取我的订单列表数据
        async getData() {
            let result = await this.$API.getMyOrder(this.page, this.limit)
            if (result.code == 200) {
                this.orderData = result.data
            }
        },
        // 点击分页器 获取当前点击的那一页 (自定义事件 接受子组件-Pagination传过来的哪一页-page)
        getpageNo(page) { // 子组件通过自定义事件传过来的page
            console.log(page)
            // 修改组件响应式数据page
            this.page = page
            // 再次发请求 获取数据渲染页面
